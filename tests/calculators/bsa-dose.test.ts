/**
 * Golden tests for calculator runner 'bsa-dose'.
 *
...
describe('bsa-dose · compute', () => {
  ...
  it('compute returns a result for mid-range inputs', () => {
    const result = calc.compute({ dosePerBSA: 2501, weight: 101, height: 125 }) as CalculatorResult | null;
  ...
  it('compute does not throw on edge values (min)', () => {
    const minInputs = { dosePerBSA: 1, weight: 2, height: 30 };
    expect(() => calc.compute(minInputs)).not.toThrow();
  });

  it('compute does not throw on edge values (max)', () => {
    const maxInputs = { dosePerBSA: 5000, weight: 200, height: 220 };
