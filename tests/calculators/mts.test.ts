...
import runner from '@/lib/runners/mts';
...
describe('mts · bands', () => {
  ...
  it('lowest band (score 1) → label matches "Красный"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/Красный/i);
  });

  it('highest band (score 5) → label matches "Синий"', () => {
    const b = findBand(bands, 5);
    expect(b.label).toMatch(/Синий/i);
  });
