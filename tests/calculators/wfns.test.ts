 ...
 *
 * Score range: 1..5, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/wfns';
...
describe('wfns · bands', () => {
  ...
  it('lowest band (score 1) → label matches "I"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/I/i);
  });

  it('highest band (score 5) → label matches "IV"', () => {
    const b = findBand(bands, 5);
    expect(b.label).toMatch(/IV/i);
  });
