 ...
 *
 * Score range: 0..30, 2 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/mace2';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('mace2 · bands', () => {
  ...
  it('lowest band (score 0) → label matches "25"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/25/i);
  });

  it('highest band (score 30) → label matches "25"', () => {
    const b = findBand(bands, 30);
    expect(b.label).toMatch(/25/i);
  });
  it('bands cover full 0..30 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(30);
