 ...
 *
 * Score range: 1..4, 5 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/ehra';
...
describe('ehra · bands', () => {
  ...
  it('lowest band (score 1) → label matches "EHRA"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/EHRA/i);
  });

  it('highest band (score 5) → label matches "EHRA"', () => {
