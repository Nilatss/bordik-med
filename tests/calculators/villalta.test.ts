/**
 * Golden tests for runner 'villalta'.
 *
 ...
 *
 * Score range: 0..34, 4 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/villalta';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('villalta · bands', () => {
  it('declares 4 band(s)', () => {
    expect(bands).toHaveLength(4);
  });

  it('every score 0..34 maps to exactly one band', () => {
    for (let s = 0; s <= 34; s++) {
  ...
  it('highest band (score 34) → label matches "15"', () => {
    const b = findBand(bands, 34);
    expect(b.label).toMatch(/15/i);
  });
  it('bands cover full 0..34 range with no overlaps', () => {
