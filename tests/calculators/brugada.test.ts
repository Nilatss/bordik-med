/**
 * Golden tests for runner 'brugada'.
 *
 ...
 *
 * Score range: 0..4, 2 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/brugada';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('brugada · bands', () => {
  ...
  it('lowest band (score 0) → label matches "СВТ"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/СВТ/i);
  });

  it('highest band (score 4) → label matches "Желудочковая"', () => {
    const b = findBand(bands, 4);
    expect(b.label).toMatch(/Желудочковая/i);
  });
  it('bands cover full 0..4 range with no overlaps', () => {
