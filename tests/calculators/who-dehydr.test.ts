/**
 * Golden tests for runner 'who-dehydr'.
 *
 ...
 *
 * Score range: 0..8, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/who-dehydr';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('who-dehydr · bands', () => {
  ...
  it('highest band (score 8) → label matches "6"', () => {
    const b = findBand(bands, 8);
    expect(b.label).toMatch(/6/i);
  });
  it('bands cover full 0..8 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(8);
