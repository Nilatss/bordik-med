/**
 * Golden tests for calculator runner 'doac'.
 *
...
describe('doac · compute', () => {
  ...
  it('compute returns a result for mid-range inputs', () => {
    const result = calc.compute({ crcl: 103, age: 64, weight: 115, scr: 515 }) as CalculatorResult | null;
  ...
  it('compute does not throw on edge values (min)', () => {
    const minInputs = { crcl: 5, age: 18, weight: 30, scr: 30 };
    expect(() => calc.compute(minInputs)).not.toThrow();
  });

  it('compute does not throw on edge values (max)', () => {
    const maxInputs = { crcl: 200, age: 110, weight: 200, scr: 1000 };
