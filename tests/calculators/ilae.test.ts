 ...
 *
 * Score range: 0..4, 1 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/ilae';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('ilae · bands', () => {
  it('declares 1 band(s)', () => {
  ...
  it('lowest band (score 0) → label matches "ILAE"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/ILAE/i);
  });

  it('highest band (score 4) → label matches "ILAE"', () => {
