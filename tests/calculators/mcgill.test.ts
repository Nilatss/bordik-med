/**
 * Golden tests for runner 'mcgill'.
 *
 ...
 *
 * Score range: 0..220, 4 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/mcgill';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('mcgill · bands', () => {
  it('declares 4 band(s)', () => {
    expect(bands).toHaveLength(4);
  });

  it('every score 0..220 maps to exactly one band', () => {
    for (let s = 0; s <= 220; s++) {
  ...
  it('highest band (score 220) → label matches "133"', () => {
    const b = findBand(bands, 220);
    expect(b.label).toMatch(/133/i);
  });
  it('bands cover full 0..220 range with no overlaps', () => {
