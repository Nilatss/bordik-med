/**
 * Golden tests for runner 'hachinski'.
 *
...
import runner from '@/lib/runners/hachinski';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('hachinski · bands', () => {
  ...
  it('lowest band (score 0) → label matches "4"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/4/i);
  });

  it('highest band (score 18) → label matches "7"', () => {
