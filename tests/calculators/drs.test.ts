 ...
 *
 * Score range: 0..29, 9 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/drs';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('drs · bands', () => {
  it('declares 9 band(s)', () => {
    expect(bands).toHaveLength(9);
  });

  it('every score 0..29 maps to exactly one band', () => {
    for (let s = 0; s <= 29; s++) {
  ...
  it('highest band (score 29) → label matches "25"', () => {
    const b = findBand(bands, 29);
    expect(b.label).toMatch(/25/i);
  });
  it('bands cover full 0..29 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(29);
