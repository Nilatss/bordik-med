/**
 * Golden tests for calculator runner 'bodyfat'.
 *
...
describe('bodyfat · compute', () => {
  ...
  it('compute returns a result for mid-range inputs', () => {
    const result = calc.compute({ height: 175, waist: 120, neck: 40, hip: 120 }) as CalculatorResult | null;
  ...
  it('compute does not throw on edge values (min)', () => {
    const minInputs = { height: 130, waist: 40, neck: 20, hip: 40 };
    expect(() => calc.compute(minInputs)).not.toThrow();
  });

  it('compute does not throw on edge values (max)', () => {
    const maxInputs = { height: 220, waist: 200, neck: 60, hip: 200 };
