/**
 * Golden tests for runner 'stanford'.
 *
 ...
 *
 * Score range: 1..3, 2 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/stanford';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('stanford · bands', () => {
  it('declares 2 band(s)', () => {
    expect(bands).toHaveLength(2);
  });

  it('every score 1..3 maps to exactly one band', () => {
    for (let s = 1; s <= 3; s++) {
  ...
  it('lowest band (score 1) → label matches "Stanford"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/Stanford/i);
  });

  it('highest band (score 3) → label matches "Stanford"', () => {
    const b = findBand(bands, 3);
    expect(b.label).toMatch(/Stanford/i);
  });
  it('bands cover full 1..3 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(1);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(3);
