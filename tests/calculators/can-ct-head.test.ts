/**
 * Golden tests for runner 'can-ct-head'.
 *
 ...
 *
 * Score range: 0..7, 2 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/can-ct-head';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('can-ct-head · bands', () => {
  ...
  it('lowest band (score 0) → label matches "Нет"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/Нет/i);
  });

  it('highest band (score 7) → label matches "1"', () => {
    const b = findBand(bands, 7);
    expect(b.label).toMatch(/1/i);
  });
  it('bands cover full 0..7 range with no overlaps', () => {
