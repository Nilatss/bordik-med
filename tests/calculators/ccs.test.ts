...
import runner from '@/lib/runners/ccs';
...
describe('ccs · bands', () => {
  ...
  it('lowest band (score 1) → label matches "CCS"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/CCS/i);
  });

  it('highest band (score 4) → label matches "CCS"', () => {
