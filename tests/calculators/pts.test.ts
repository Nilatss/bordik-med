 ...
 *
 * Score range: -6..12, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/pts';
...
describe('pts · bands', () => {
  ...
  it('every score -6..12 maps to exactly one band', () => {
    for (let s = -6; s <= 12; s++) {
  ...
  it('lowest band (score -6) → label matches "0"', () => {
    const b = findBand(bands, -6);
    expect(b.label).toMatch(/0/i);
  });

  it('highest band (score 12) → label matches "9"', () => {
    const b = findBand(bands, 12);
    expect(b.label).toMatch(/9/i);
  });
  it('bands cover full -6..12 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(-6);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(12);
