 ...
 *
 * Score range: 0..27, 4 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/crib';
...
describe('crib · bands', () => {
  ...
  it('highest band (score 27) → label matches "16"', () => {
    const b = findBand(bands, 27);
    expect(b.label).toMatch(/16/i);
  });
  it('bands cover full 0..27 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(27);
