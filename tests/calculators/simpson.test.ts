/**
 * Golden tests for runner 'simpson'.
 *
 ...
 *
 * Score range: 1..5, 5 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/simpson';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('simpson · bands', () => {
  ...
  it('lowest band (score 1) → label matches "Simpson"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/Simpson/i);
  });

  it('highest band (score 5) → label matches "Simpson"', () => {
    const b = findBand(bands, 5);
    expect(b.label).toMatch(/Simpson/i);
  });
  it('bands cover full 1..5 range with no overlaps', () => {
