/**
 * Golden tests for runner 'h2fpef'.
 *
 ...
 *
 * Score range: 0..9, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/h2fpef';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('h2fpef · bands', () => {
  ...
  it('highest band (score 9) → label matches "6"', () => {
    const b = findBand(bands, 9);
    expect(b.label).toMatch(/6/i);
  });
  it('bands cover full 0..9 range with no overlaps', () => {
