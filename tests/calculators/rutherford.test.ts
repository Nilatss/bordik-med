/**
 * Golden tests for runner 'rutherford'.
 *
 ...
 *
 * Score range: 0..6, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/rutherford';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('rutherford · bands', () => {
  ...
  it('lowest band (score 0) → label matches "R0"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/R0/i);
  });

  it('highest band (score 6) → label matches "R4"', () => {
    const b = findBand(bands, 6);
    expect(b.label).toMatch(/R4/i);
  });
  it('bands cover full 0..6 range with no overlaps', () => {
