...
import runner from '@/lib/runners/acdu';
...
describe('acdu · bands', () => {
  ...
  it('lowest band (score 4) → label matches "A"', () => {
    const b = findBand(bands, 4);
    expect(b.label).toMatch(/A/i);
  });

  it('highest band (score 1) → label matches "U"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/U/i);
  });
