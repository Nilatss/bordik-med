 ...
 *
 * Score range: 0..2, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/sga';
...
describe('sga · bands', () => {
  ...
  it('every score 0..2 maps to exactly one band', () => {
    for (let s = 0; s <= 2; s++) {
  ...
  it('lowest band (score 0) → label matches "SGA"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/SGA/i);
  });

  it('highest band (score 2) → label matches "SGA"', () => {
    const b = findBand(bands, 2);
    expect(b.label).toMatch(/SGA/i);
  });
  it('bands cover full 0..2 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(2);
