...
import runner from '@/lib/runners/bova';
...
describe('bova · bands', () => {
  ...
  it('lowest band (score 0) → label matches "Стадия"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/Стадия/i);
  });

  it('highest band (score 7) → label matches "Стадия"', () => {
