/**
 * Golden tests for runner 'finnegan'.
 *
 ...
 *
 * Score range: 0..46, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/finnegan';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('finnegan · bands', () => {
  it('declares 3 band(s)', () => {
    expect(bands).toHaveLength(3);
  });

  it('every score 0..46 maps to exactly one band', () => {
    for (let s = 0; s <= 46; s++) {
  ...
  it('highest band (score 46) → label matches "12"', () => {
    const b = findBand(bands, 46);
    expect(b.label).toMatch(/12/i);
  });
  it('bands cover full 0..46 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(46);
