...
import runner from '@/lib/runners/engel';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('engel · bands', () => {
  ...
  it('lowest band (score 1) → label matches "Класс"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/Класс/i);
  });

  it('highest band (score 4) → label matches "Класс"', () => {
