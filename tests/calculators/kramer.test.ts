/**
 * Golden tests for runner 'kramer'.
 *
...
import runner from '@/lib/runners/kramer';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('kramer · bands', () => {
  ...
  it('lowest band (score 1) → label matches "Зона"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/Зона/i);
  });

  it('highest band (score 5) → label matches "Зона"', () => {
