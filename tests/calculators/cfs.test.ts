 ...
 *
 * Score range: 1..9, 4 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/cfs';
...
describe('cfs · bands', () => {
  ...
  it('every score 1..9 maps to exactly one band', () => {
    for (let s = 1; s <= 9; s++) {
  ...
  it('lowest band (score 1) → label matches "1"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/1/i);
  });

  it('highest band (score 9) → label matches "7"', () => {
    const b = findBand(bands, 9);
    expect(b.label).toMatch(/7/i);
  });
  it('bands cover full 1..9 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(1);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(9);
