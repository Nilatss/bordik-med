 ...
 *
 * Score range: 0..10, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/slic';
...
describe('slic · bands', () => {
  ...
  it('lowest band (score 0) → label matches "3"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/3/i);
  });

  it('highest band (score 10) → label matches "5"', () => {
    const b = findBand(bands, 10);
    expect(b.label).toMatch(/5/i);
  });
  it('bands cover full 0..10 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(10);
