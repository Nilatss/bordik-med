 ...
 *
 * Score range: 0..30, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/slums';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('slums · bands', () => {
  ...
  it('highest band (score 30) → label matches "27"', () => {
    const b = findBand(bands, 30);
    expect(b.label).toMatch(/27/i);
  });
