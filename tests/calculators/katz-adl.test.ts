/**
 * Golden tests for runner 'katz-adl'.
 *
...
import runner from '@/lib/runners/katz-adl';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('katz-adl · bands', () => {
  ...
  it('lowest band (score 6) → label matches "6"', () => {
    const b = findBand(bands, 6);
    expect(b.label).toMatch(/6/i);
  });

  it('highest band (score 2) → label matches "0"', () => {
