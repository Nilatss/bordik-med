 ...
 *
 * Score range: 0..6, 7 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/ceap';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('ceap · bands', () => {
  it('declares 7 band(s)', () => {
  ...
  it('lowest band (score 0) → label matches "C0"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/C0/i);
  });

  it('highest band (score 6) → label matches "C6"', () => {
    const b = findBand(bands, 6);
    expect(b.label).toMatch(/C6/i);
  });
