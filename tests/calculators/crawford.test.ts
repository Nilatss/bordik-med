/**
 * Golden tests for runner 'crawford'.
 *
...
import runner from '@/lib/runners/crawford';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('crawford · bands', () => {
  ...
  it('lowest band (score 1) → label matches "Тип"', () => {
    const b = findBand(bands, 1);
    expect(b.label).toMatch(/Тип/i);
  });

  it('highest band (score 5) → label matches "Тип"', () => {
