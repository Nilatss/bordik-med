/**
 * Golden tests for runner 'epworth'.
 *
 ...
 *
 * Score range: 0..24, 5 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/epworth';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('epworth · bands', () => {
  ...
  it('highest band (score 24) → label matches "16"', () => {
    const b = findBand(bands, 24);
    expect(b.label).toMatch(/16/i);
  });
  it('bands cover full 0..24 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(24);
