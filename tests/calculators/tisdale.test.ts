/**
 * Golden tests for runner 'tisdale'.
 *
 ...
 *
 * Score range: 0..21, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/tisdale';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('tisdale · bands', () => {
  ...
  it('lowest band (score 0) → label matches "6"', () => {
    const b = findBand(bands, 0);
    expect(b.label).toMatch(/6/i);
  });

  it('highest band (score 21) → label matches "11"', () => {
    const b = findBand(bands, 21);
    expect(b.label).toMatch(/11/i);
  });
  it('bands cover full 0..21 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(21);
