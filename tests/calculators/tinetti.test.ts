/**
 * Golden tests for runner 'tinetti'.
 *
 ...
 *
 * Score range: 0..28, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/tinetti';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('tinetti · bands', () => {
  it('declares 3 band(s)', () => {
    expect(bands).toHaveLength(3);
  });

  it('every score 0..28 maps to exactly one band', () => {
    for (let s = 0; s <= 28; s++) {
  ...
  it('lowest band (score 25) → label matches "25"', () => {
    const b = findBand(bands, 25);
    expect(b.label).toMatch(/25/i);
  });

  it('highest band (score 18) → label matches "18"', () => {
    const b = findBand(bands, 18);
    expect(b.label).toMatch(/18/i);
  });
  it('bands cover full 0..28 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(28);
