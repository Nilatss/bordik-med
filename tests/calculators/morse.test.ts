 ...
 *
 * Score range: 0..125, 3 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/morse';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('morse · bands', () => {
  it('declares 3 band(s)', () => {
    expect(bands).toHaveLength(3);
  });

  it('every score 0..125 maps to exactly one band', () => {
    for (let s = 0; s <= 125; s++) {
  ...
  it('highest band (score 125) → label matches "45"', () => {
    const b = findBand(bands, 125);
    expect(b.label).toMatch(/45/i);
  });
  it('bands cover full 0..125 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(125);
