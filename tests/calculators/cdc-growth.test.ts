/**
 * Golden tests for calculator runner 'cdc-growth'.
 *
...
describe('cdc-growth · compute', () => {
  ...
  it('compute returns a result for mid-range inputs', () => {
    const result = calc.compute({ ageYears: 11, weight: 104, height: 145 }) as CalculatorResult | null;
  ...
  it('compute does not throw on edge values (min)', () => {
    const minInputs = { ageYears: 2, weight: 8, height: 70 };
    expect(() => calc.compute(minInputs)).not.toThrow();
  });

  it('compute does not throw on edge values (max)', () => {
    const maxInputs = { ageYears: 20, weight: 200, height: 220 };
