/**
 * Golden tests for runner 'thompson'.
 *
 ...
 *
 * Score range: 0..22, 4 bands.
 */
import { describe, it, expect } from 'vitest';
import runner from '@/lib/runners/thompson';
import { findBand, type ScoreBand } from '@/lib/tools-runners';

const bands = (runner as { bands: ScoreBand[] }).bands;

describe('thompson · bands', () => {
  it('declares 4 band(s)', () => {
    expect(bands).toHaveLength(4);
  });

  it('every score 0..22 maps to exactly one band', () => {
    for (let s = 0; s <= 22; s++) {
  ...
  it('highest band (score 22) → label matches "15"', () => {
    const b = findBand(bands, 22);
    expect(b.label).toMatch(/15/i);
  });
  it('bands cover full 0..22 range with no overlaps', () => {
    const sorted = [...bands].sort((a, b) => a.min - b.min);
    expect(sorted[0]?.min).toBeLessThanOrEqual(0);
    expect(sorted[sorted.length - 1]?.max).toBeGreaterThanOrEqual(22);
