...
| 3 | МКБ-10 lookup | ✅ выкачен (3742 кодов, v0.6.0) | S | done |
| 3a | **Унифицированный хаб классификаций (МКБ/ICD)** | 🔴 не начато | L | 2-3 недели |
...
## #3a. Унифицированный хаб классификаций (МКБ/ICD)

**Что делать:** объединить все варианты ICD/МКБ под одним разделом
"Классификации" в сервисах (sidebar). Пользователь выбирает страну /
ревизию, видит соответствующий код, может переключаться между системами
для одного диагноза (cross-walk).

**Эталон:** WHO ICD-Browser (icd.who.int) + AAPC ICD-10-CM lookup.

### Почему это L (а не M)

- 7 разных классификаций, каждая со своей структурой и ~5-15 тыс. кодов.
- Cross-mapping между ICD-10 ↔ ICD-11 ↔ ICD-10-CM требует таблиц
  соответствия (WHO предоставляет частичные).
- ICD-10-PCS — это не диагнозы, а процедуры (7-значные коды), нужна
  отдельная UX-модель.
- Локализация: МКБ-10 (РФ) на русском, остальные — на английском
  (или с частичными переводами).

### Структура раздела (UI/IA)

В sidebar `Сервисы → Классификации` — единая страница со страничным
переключателем. Каждая система — отдельная вкладка/таб.

```
Классификации
├── МКБ-10 (Россия, ВОЗ rev.10)         ← 3742 кода, есть
├── МКБ-11 (Россия, ВОЗ rev.11)         ← TODO
├── ICD-10-CM (США, клиническая)         ← TODO
├── ICD-10-PCS (США, процедуры)          ← TODO (отдельная модель)
├── ICD-10-CA (Канада)                   ← TODO
├── ICD-10-GM (Германия)                 ← TODO
└── ICD-10-AM (Австралия/NZ)             ← TODO
```

### Сравнительная таблица систем

| Система | Регион | Кодов (~) | Источник | Лицензия |
|---|---|---|---|---|
| **МКБ-10** | РФ, СНГ | 14 400 | Минздрав РФ (3-значн. адаптация) | Public domain (RU) |
| **МКБ-11** | Мир (с 2022) | 17 000 + extension codes | WHO | CC BY-ND 3.0 IGO |
| **ICD-10-CM** | США (диагнозы) | 72 750 | CDC/NCHS | Public domain (US) |
| **ICD-10-PCS** | США (процедуры) | 78 000 | CMS | Public domain (US) |
| **ICD-10-CA** | Канада | 17 100 | CIHI | Лицензионный (CIHI) |
| **ICD-10-GM** | Германия | 16 000 | BfArM/DIMDI | Public (DE) |
| **ICD-10-AM** | Австралия, NZ, Ирландия, Сингапур | 16 800 | IHACPA | Лицензионный |

### Этапы (декомпозиция)

**Этап 1 (1 нед, M) — IA + переезд существующего**
- [ ] Создать `app/services/classifications/` с табами
- [ ] Перенести МКБ-10 lookup внутрь хаба (без потери deeplink на `/icd10`)
- [ ] Сайдбар: "МКБ-10" → "Классификации" с активным табом МКБ-10
- [ ] Хлебные крошки: Сервисы → Классификации → МКБ-10
- [ ] Cross-link UI заглушка (placeholder таблицы)

**Этап 2 (3-5 дней, S) — МКБ-11 (приоритет)**
- [ ] Скачать MMS Linearization JSON с icd.who.int (открытый API)
- [ ] Адаптер `data/icd11.json` (~17k кодов)
- [ ] Таб "МКБ-11" с тем же UX, что и МКБ-10
- [ ] Cross-walk МКБ-10 ↔ МКБ-11 (WHO mapping table)

**Этап 3 (5-7 дней, M) — ICD-10-CM (США)**
- [ ] Скачать FY2025 ICD-10-CM Code Tables (CDC, public domain)
- [ ] `data/icd10-cm.json` (~73k кодов)
- [ ] UX отличия: 7-значная детализация (этиология, локализация,
      сторона, инициальный/последующий визит)
- [ ] Базовый поиск по коду + английскому descrption
- [ ] Cross-walk ICD-10 ↔ ICD-10-CM (CMS GEM mapping)

**Этап 4 (3-5 дней, S) — ICD-10-GM (Германия)**
- [ ] Скачать с BfArM (public domain в DE)
- [ ] `data/icd10-gm.json` (~16k кодов, DE labels)
- [ ] Таб "ICD-10-GM"

**Этап 5 (5-7 дней, L) — ICD-10-PCS (процедуры)**
- [ ] Отдельная UX-модель: 7-значные процедурные коды
- [ ] Селектор по разделам (Medical/Surgical, Imaging, etc.)
- [ ] Body system → root operation → body part → approach → device → qualifier
- [ ] Скачать FY2025 PCS Code Tables (CMS)

**Этап 6 (5-7 дней, M) — ICD-10-CA / ICD-10-AM**
- [ ] CIHI и IHACPA — лицензии платные. Решение: либо API-stub
      "доступно по запросу клиники", либо переговоры о data-license.
- [ ] Если лицензия — таб с full lookup; иначе — публичная заглушка
      "контент по подписке организации".

**Этап 7 (3-5 дней) — Cross-walk и поиск**
- [ ] Универсальный поиск через все 7 систем одновременно
- [ ] Карточка кода с эквивалентами в других системах (если mapping есть)
- [ ] Экспорт диагноза в формате нужной страны

### Критерии готовности (DoD)

- Все 7 классификаций доступны как табы в одном разделе.
- Поиск работает по коду и описанию на нативном языке системы.
- Service Worker кэширует JSON через `?v=` (cache-busting).
- 0 дублей кодов в каждой системе (validation script в CI).
- Cross-walk между минимум 3 системами (МКБ-10 ↔ ICD-10-CM ↔ МКБ-11).
- BETA-баннер на ICD-10-CA/AM до решения с лицензиями.

### Открытые вопросы / риски

- **Лицензии CIHI / IHACPA** — нужно решение бизнес-уровня.
- **Размер JSON**: ICD-10-CM ~73k кодов = ~5-10 MB; нужен code-splitting
  по главам или server-side search (Postgres + tsvector).
- **МКБ-11 на русском**: WHO даёт частичный перевод, остальное — EN.
- **Mapping FK между системами** — нет 1:1 в большинстве случаев,
  нужно показывать "одно ко многим" с пометкой "approximate".
